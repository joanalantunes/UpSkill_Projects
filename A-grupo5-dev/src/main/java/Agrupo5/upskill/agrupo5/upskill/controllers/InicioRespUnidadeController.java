...
@Controller
public class InicioRespUnidadeController {

    @Autowired
    UsersService usersService;

    @GetMapping(value = "/inicio_respunidade/{id}")
    public ModelAndView inicio_respunidade(@PathVariable Long id, ModelMap map) {
        User dbUser = usersService.getUserById(id);
        map.put("user", dbUser);

        ModelAndView mav = new ModelAndView("inicio_respunidade");
        return mav;
    }
}
